    ...
    try:
        ...
        if feature == "creative_generation":
            if creative_type == "current_temperature":
                try:  # Try to extract temperature info; provide error message if it fails.
                    temperature_info = extract_temperature(gemini_response)
                    gemini_response["temperature"] = temperature_info

                except (KeyError, IndexError, TypeError, ValueError) as e:
                    gemini_response["temperature_error"] = f"Could not extract temperature: {e}"  # Clear error message
